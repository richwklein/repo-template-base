...
1. Save the ruleset JSON above to `ruleset.json` (adjust `required_status_checks` to match the workflows this repo actually runs — e.g., drop `test` for repos without a test suite).
2. Apply via `gh`:
